...
function updateSummary(departments) {
  ...
  departments.forEach(dept => {
    if (dept.total) {
      totalEnergy += dept.total.total_energy || 0;
      totalCost += dept.total.total_cost || 0;
    }
  });
...
function updateLastUpdate() {
  ...
  if (lastUpdateEl) {
    const date = new Date();
    lastUpdateEl.textContent = `Terakhir update: ${date.toLocaleString('id-ID')}`;
  }
}
...
function showLoading() {
  ...
  if (!container.querySelector('.loading-indicator')) {
    ...
    `;
    loading.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Memuat data...';
...
function showSuccess() {
  ...
  successEl.innerHTML = '<i class="fa-solid fa-check-circle"></i> Data berhasil diperbarui';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'r' || e.key === 'R') {
      if (!e.ctrlKey && !e.metaKey) { // Hindari conflict dengan Ctrl+R
        showLoading();
        fetchData();
      }
    }
